...
\begin{itemize}
  ...
  \begin{itemize}
    \resitem{ {\bf 项目介绍:} Bvcflow任务编排工作流系统是B站稿件生产的核心流程系统，对用户上传的视频稿件，按照预设的顺序完成视频的检测、截图、转码、分发等计算任务，支撑单日百万量级稿件的生产。}
    \resitem{ {\bf 职责描述:}
      \begin{itemize}
        % \item 开发与维护
        \item 实现任务编排从单个任务依次串行执行到多任务并行执行的改造，支持以DAG描述任务间的依赖关系，提高生产流程的执行效率。
        \item 优化任务状态机模型和实现，优化mongo的热点更新问题和查询索引，从而降低mongo负载，提升任务并发度，并支持横向扩展。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{itemize}
  \item {\bf 编程语言:} Golang(熟练), OpenResty/Lua(熟练), Python(熟练), C++(掌握), Qt(掌握), Java(了解)
  \item {\bf OS:} Linux(熟练), Windows(掌握)
  \item {\bf 其他:} Django, PostgreSQL, Nginx, Docker, Jenkins, Git, Vue
